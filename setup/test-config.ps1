...
try {
    ...
    $requiredKeys = @('ProxyIP', 'ControlPlaneIPs', 'WorkerIPs', 'SSHUser', 'SSHKeyPath', 'K3sVersion', 'ServiceCIDR', 'ClusterCIDR')
    ...
    Write-Host "    ssh -i $($Config.SSHKeyPath) -o StrictHostKeyChecking=no $($Config.SSHUser)@$($Config.ControlPlaneIPs[0]) $testCommand" -ForegroundColor Cyan
